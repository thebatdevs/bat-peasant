...
```ts
import type { OptPrtEntity, QueriedEntitys } from '@/repository';
import {
  getEntityById as getEntityByIdRepo,
  getEntitysByQuery as getEntitysByQueryRepo,
...
/**
 * Queries Entitys based on provided conditions.
 */
export async function getEntitysByQuery(query: DynamoQueryRequest, proj?: string): Promise<QueriedEntitys> {
  try {
    return await getEntitysByQueryRepo(query, proj);
  } catch (error: unknown) {
    logErrorMessage(error, 'getEntitysByQuery');
    throw error;
  }
}
...
export async function getEntityByIndex(indexValue: string): Promise<OptPrtEntity> {
  try {
    ...
    const { items } = await getEntitysByQuery(query);
