...
Finish v1.1.0-codex as an installable, internally consistent Codex context system whose activation, selective context loading, planning, execution, and validation workflows can be followed using files that actually ship in the version. Release completion requires restoring the established canonical `.agent/` directory name, resolving the owner-policy questions below, replacing scaffolds with usable content, reconciling existing rules/examples/skills without weakening valid rules, and validating the package in a representative repository.

> **Path note after the second audit:** The current working tree stores the 29 persistent-context files under `.agents/`; all are byte-identical to their former `.agent/` versions. Downstream task paths use `.agent/` as the required post-BP-001 destination. Until BP-001 is complete, inspect the corresponding current file by replacing the `.agent/` prefix with `.agents/`.

## Audit Summary

### Current State

The version contains a concise `AGENTS.md`, substantial TypeScript Serverless monorepo metadata, one detailed coding rule, four specialized DynamoDB skills, and four code examples. The implementation has been renamed from `.agent/` to `.agents/`, and `AGENTS.md`, `README.md`, and `.agents/info.md` now consistently resolve inside that renamed tree. This fixes the former broken-reference symptom, but it does not satisfy the established v1.1.0-codex requirement that `.agent/` is the persistent project-context directory. The context and plan locations still do not exist, most prompts and technical docs are empty, two rule files and `requirements.md` are headings only, and the release has no installation or end-to-end validation procedure.

The `variants/` tree is a second, undocumented source tree. It duplicates many `.agent/` files, contains empty alternate rules/prompts/docs, and is the only location for single-project TypeScript Serverless metadata. Its intended release role is not defined.
...
- The entire persistent-context tree and all workflow references use `.agents/`, contrary to the established canonical `.agent/` requirement.
- `.agent/info.md` omits activation and several required selection, maintenance, conflict, and unsupported-assumption policies.
- `.agent/context/` and `.agent/plans/` are absent, and their commit/freshness/lifecycle policies are unresolved.
- Seven prompt files are empty and two feature prompts contain only a heading.
...
- `AGENTS.md:17` now reaches `.agents/info.md`, but both the reference and the directory violate the required `.agent/` contract; installations following the established layout will not match the shipped entry point.
- Plan creation is directed to nonexistent `.agents/plans/`, while the established location is `.agent/plans/`.
...
- Every shipped workflow and persistent-context file uses the established `.agent/` directory consistently, and all referenced paths exist in an installed copy.
- `AGENTS.md` remains below approximately 500 tokens and delegates detailed behavior to `.agent/info.md`.
- `.agent/info.md` fully defines activation, priority, selective loading, direct-versus-planned execution, approval, validation, completion, source selection, context maintenance, conflict handling, and assumption handling.
...
## Phase 1 — Critical Structure and Workflow

### BP-001 — Restore the canonical `.agent/` directory and paths
...
- **Problem:** The former broken references were made reachable by renaming all 29 `.agent/` files to `.agents/`. That creates internal consistency, but contradicts the established v1.1.0-codex requirement that `.agent/` is the persistent project-context directory and leaves plan creation aimed at an absent `.agents/plans/`.
- **Required result:** Rename the persistent-context tree back to `.agent/`, update all workflow references to `.agent/`, and preserve the exact `##bat-peasant` activation contract. Do not treat internal consistency with the unsupported plural name as requirement completion.
- **Dependencies:** `None`
- **Acceptance criteria:**
  - [ ] The 29 current `.agents/` files exist under `.agent/` with no content loss.
  - [ ] No `.agents/` reference remains anywhere in the version.
  - [ ] Every documented `.agent/` target exists or is created by a separately documented installation/runtime step.
  - [ ] `AGENTS.md` directs activated Codex to `.agent/info.md` and directs complex plans to `.agent/plans/<task-name>.plan.md`.
...
- **Evidence:**
  ...
  - The current working tree contains `.agents/` and no `.agent/` directory.
  - A byte comparison against the pre-rename Git files found 29 identical files and no content changes.
...
- **Affected paths:**
  - `models/v1.1.0-codex/AGENTS.md`
  - `models/v1.1.0-codex/.agent/info.md`
  - `models/v1.1.0-codex/README.md`
...
- **Dependencies:** `BP-001`
- **Acceptance criteria:**
  - [ ] `.agent/info.md` defines the complete precedence once and explains how source applicability affects it.
  - [ ] `AGENTS.md` remains concise and points to `.agent/info.md` for detailed priority rather than duplicating it.
...
- **Evidence:**
  - `models/v1.1.0-codex/.agent/info.md:114-130`
...
- **Required result:** Make `.agent/info.md` the complete canonical workflow while keeping `AGENTS.md` a sub-500-token activation and routing entry point.
- **Dependencies:** `BP-001`, `BP-002`
- **Acceptance criteria:**
  - [ ] `.agent/info.md` states exact-marker activation and normal inactive behavior.
...
- **Evidence:**
  - `models/v1.1.0-codex/.agent/info.md:1-259`
  - `models/v1.1.0-codex/AGENTS.md` is 101 words and contains activation, but `.agent/info.md` contains no `##bat-peasant` marker or core-principle text.
...
- **Category:** Structure
- **Affected paths:**
  - `models/v1.1.0-codex/.agent/`
  - `models/v1.1.0-codex/.agent/info.md`
  - `models/v1.1.0-codex/variants/`
- **Problem:** `.agent/context/` and `.agent/plans/` are documented but absent. No runtime/temporary location is documented. `variants/` duplicates the source layout without ownership or install semantics, making it unclear which files are canonical or reachable.
...
- **Acceptance criteria:**
  - [ ] Every required directory documented in `.agent/info.md` exists in the distributable package.
...
- **Evidence:**
  - `models/v1.1.0-codex/.agent/info.md:13-23` documents `context/` and `plans/`.
  - Neither `models/v1.1.0-codex/.agent/context/` nor `models/v1.1.0-codex/.agent/plans/` exists.
  - `models/v1.1.0-codex/variants/` mirrors `.agent/` but is not mentioned by `AGENTS.md`, `.agent/info.md`, `README.md`, or `requirements.md`.

## Phase 2 — Core Documentation and Metadata
...
- **Affected paths:**
  - `models/v1.1.0-codex/.agent/info.md`
  - `models/v1.1.0-codex/.agent/context/`
...
- **Evidence:**
  - `models/v1.1.0-codex/.agent/info.md:39-50`
  - No `.agent/context/` content or context-specific template exists.
...
- **Acceptance criteria:**
  ...
  - [ ] It links rather than duplicates detailed `.agent/info.md` workflow text.
...
- **Evidence:**
  ...
  - Root `README.md:7`, `README.md:31-49` describes `code/.bat-peasant/`, not `.agent/`.
...
- **Affected paths:**
  - `models/v1.1.0-codex/.agent/info.md`
  - `models/v1.1.0-codex/.agent/metadata/`
- **Problem:** The version has only a TypeScript Serverless monorepo metadata pair in `.agent/`; no naming contract, metadata schema, profile matching rules, applicability/fallback behavior, validation, or reusable template is documented.
...
- **Evidence:**
  - `models/v1.1.0-codex/.agent/metadata/` contains only `typescript-serverless-monorepo.project-structure.md` and `.tech-stack.md`.
  - `.agent/info.md:27-37` describes metadata purpose but not discovery or selection.
...
- **Category:** Metadata
- **Affected paths:**
  - `models/v1.1.0-codex/.agent/metadata/typescript-serverless-monorepo.project-structure.md`
  - `models/v1.1.0-codex/.agent/metadata/typescript-serverless-monorepo.tech-stack.md`
...
- **Evidence:**
  - `models/v1.1.0-codex/.agent/metadata/typescript-serverless-monorepo.project-structure.md:8-40`
...
## Phase 3 — Prompts, Rules, Examples, and Skills
...
- **Category:** Prompt
- **Affected paths:**
  - `models/v1.1.0-codex/.agent/prompts/feature/implement-new-feature.md`
  - `models/v1.1.0-codex/.agent/prompts/feature/update-existing-feature.md`
  - `models/v1.1.0-codex/.agent/prompts/maintenance/context-documentation.md`
  - `models/v1.1.0-codex/.agent/prompts/maintenance/debug.md`
  - `models/v1.1.0-codex/.agent/prompts/maintenance/migration.md`
  - `models/v1.1.0-codex/.agent/prompts/maintenance/refactor.md`
  - `models/v1.1.0-codex/.agent/prompts/maintenance/review-pull-request.md`
...
- **Category:** Prompt
- **Affected paths:**
  - `models/v1.1.0-codex/.agent/prompts/`
  - `models/v1.1.0-codex/.agent/info.md`
...
- **Evidence:**
  - `.agent/info.md:88-99` lists only seven task types.
  - `.agent/prompts/` has no plan, validation, structure-audit, or knowledge-source maintenance workflow.
...
- **Category:** Rule
- **Affected paths:**
  - `models/v1.1.0-codex/.agent/rules/typescript-serverless.md`
  - `models/v1.1.0-codex/.agent/rules/typescript-testing.md`
  - `models/v1.1.0-codex/.agent/rules/typescript-coding-standard.md`
  - `models/v1.1.0-codex/.agent/metadata/typescript-serverless-monorepo.project-structure.md`
...
- **Affected paths:**
  - `models/v1.1.0-codex/.agent/info.md`
  - `models/v1.1.0-codex/.agent/rules/`
...
- **Evidence:**
  - `.agent/info.md:51-62` lists rule topics but no discovery or conflict algorithm.
...
- **Category:** Documentation
- **Affected paths:**
  - `models/v1.1.0-codex/.agent/docs/custom-axios.md`
  - `models/v1.1.0-codex/.agent/docs/dynamo-command-builder.md`
  - `models/v1.1.0-codex/.agent/docs/package-lib.md`
  - `models/v1.1.0-codex/.agent/docs/package-schema.md`
  - `models/v1.1.0-codex/.agent/skills/dynamo-command-builder/references/api.md`
- **Problem:** All four `.agent/docs/` files are empty, yet `.agent/info.md` tells Codex not to guess when internal documentation exists. Dynamo command-builder documentation also overlaps a substantive skill reference without defining which is canonical.
...
- **Evidence:**
  - All four affected `.agent/docs/*.md` files have zero bytes.
  - `.agent/info.md:76-86` says to use docs for internal APIs and not guess.
...
- **Category:** Example
- **Affected paths:**
  - `models/v1.1.0-codex/.agent/examples/dynamo-client.functions.md`
  - `models/v1.1.0-codex/.agent/examples/dynamo-repository.functions.md`
  - `models/v1.1.0-codex/.agent/examples/dynamo-service.functions.md`
  - `models/v1.1.0-codex/.agent/examples/project-structure-brief.example.md`
  - `models/v1.1.0-codex/.agent/rules/typescript-coding-standard.md`
  - `models/v1.1.0-codex/.agent/metadata/typescript-serverless-monorepo.project-structure.md`
...
- **Category:** Example
- **Affected paths:**
  - `models/v1.1.0-codex/.agent/examples/`
  - `models/v1.1.0-codex/.agent/info.md`
...
- **Evidence:**
  - `.agent/info.md:64-74` calls examples highest-signal but gives no selection method.
...
- **Category:** Skill
- **Affected paths:**
  - `models/v1.1.0-codex/.agent/skills/create-dynamo-service-function/SKILL.md`
  - `models/v1.1.0-codex/.agent/skills/create-dynamo-service-function/agents/openai.yaml`
  - `models/v1.1.0-codex/.agent/examples/dynamo-service.functions.md`
  - `models/v1.1.0-codex/.agent/metadata/typescript-serverless-monorepo.project-structure.md`
...
- **Category:** Skill
- **Affected paths:**
  - `models/v1.1.0-codex/.agent/skills/create-dynamo-client-dash-style/SKILL.md`
  - `models/v1.1.0-codex/.agent/skills/create-dynamo-repository-dash-style/SKILL.md`
  - `models/v1.1.0-codex/.agent/examples/dynamo-client.functions.md`
  - `models/v1.1.0-codex/.agent/examples/dynamo-repository.functions.md`
  - `models/v1.1.0-codex/.agent/metadata/typescript-serverless-monorepo.project-structure.md`
...
- **Evidence:**
  - `.agent/info.md:110-112` is the complete current skill guidance.
...
- **Affected paths:**
  - `models/v1.1.0-codex/.agent/info.md`
  - `models/v1.1.0-codex/.agent/plans/`
  - `models/v1.1.0-codex/.agent/prompts/`
...
- **Evidence:**
  - `.agent/info.md:100-108,171-194`
  - `.agent/plans/` and a plan template are absent.
...
- **Acceptance criteria:**
  ...
  - [ ] It rejects `.agents/`, stale release versions, missing required paths, and zero-byte files advertised as usable.
...
- **Acceptance criteria:**
  ...
  - [ ] A task with the exact marker demonstrates activation and reads `.agent/info.md`.
...
- **Evidence:**
  ...
  - `.agent/info.md` currently lacks activation scenarios.
...
- **Evidence:**
  ...
  - `models/v1.0.0/` uses `.ai/context`, `decisions`, `maintenance`, `standards`, `testing`, and `workflow`, which do not map one-to-one to `.agent/`.
...
- **Question:** Should `.agent/context/` be committed, ignored, or selectively committed, and how are ownership, freshness, replacement/append behavior, history, and conflicts handled?
...
- **Affected paths:**
  - `models/v1.1.0-codex/.agent/context/`
  - `models/v1.1.0-codex/.agent/info.md`
- **Available evidence:** `.agent/info.md` describes context as business/domain knowledge but the directory is absent. The root v1.0.0 concept treats context as maintained repository memory, while the current version gives no source-control or freshness rule.
- **Options:**
...
- **Affected paths:**
  - `models/v1.1.0-codex/.agent/plans/`
  - `models/v1.1.0-codex/.agent/info.md`
- **Available evidence:** `.agent/info.md:108` allows either selective commit or ignore without selection criteria. Established requirements require plans for complex work but intentionally do not define lifecycle policy.
- **Options:**
...
- **Why it matters:** The tree shadows `.agent/`, includes unique single-project metadata, and contains empty or divergent copies; without a decision, Codex and maintainers cannot know which source is authoritative.
- **Affected paths:**
  - `models/v1.1.0-codex/variants/`
  - `models/v1.1.0-codex/.agent/`
  - `models/v1.1.0-codex/README.md`
- **Available evidence:** Many variant files are byte-identical to `.agent/`; others differ; single-project metadata exists only in `variants/`; no version document references the directory.
- **Options:**
  1. Treat variants as installable profiles and document a deterministic materialization process into `.agent/`.
  2. Treat variants as development-only reference inputs and exclude them from installed/release authority.
  3. Consolidate authoritative supported content under `.agent/` and remove duplication in a future implementation task after preserving unique valid content.
- **Blocked tasks:**
  - `BP-004`
  - `BP-006`
  - `BP-009`
...
- **Affected paths:**
  ...
  - `models/v1.1.0-codex/README.md`
  - `models/v1.1.0-codex/requirements.md`
- **Available evidence:** `.agent/metadata/` contains only a monorepo pair; `variants/metadata/` adds a single-project pair; the root project describes language-agnostic goals, while current rules/examples/skills are TypeScript/DynamoDB-specific.
- **Options:**
